...
class AsyncNLPProcess(threading.Thread):
        ...
        for k in tqdm(range(0, len(l1))):
            ...
            if (len(l) > 0):
                for i in range(0, len(l)):
                    ...
                    tempdict["sentence"] = self.tok_text(obj0.sentences.loc[k])
        ...
        for sent in nltk.sent_tokenize(txt):
            for chunk in nltk.ne_chunk(nltk.pos_tag(nltk.word_tokenize(sent))):
                if hasattr(chunk, 'label'):
                    ...
                    tempdict['sentence'] = self.tok_text(txt)
    ...
    def text_spacy_nc(self, txt, name, idx):
        ...
        ncdf['sentence'] = self.tok_text(txt)
    ...
    def run(self):
        ...
        df_topic_sents_keywords = self.format_topics_sentences(ldamodel=lda_model, corpus=corpus, texts=data)
        # # Format
