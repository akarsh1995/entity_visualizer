...
np.random.seed(1)
RESOURCES_FOLDER = os.path.join(config.model_data, "resources")
module_location = os.path.abspath(__file__)
module_location = os.path.dirname(module_location)
   

property_blacklist = embeddings.load_blacklist(os.path.join(RESOURCES_FOLDER, "labellist","property_blacklist.txt"))
property2idx = {}
with open(os.path.join(RESOURCES_FOLDER, "labellist", "property2idx.txt")) as f:
    property2idx = ast.literal_eval(f.read())
idx2property = {v: k for k, v in property2idx.items()}

_, position2idx = embeddings.init_random(np.arange(-config.Params.max_sent_len, config.Params.max_sent_len),
...
def model_ContextSum(embedding_matrix, max_sent_len, n_out):
    print(config.params_dict)
    ...
    pos_embeddings = layers.wrappers.TimeDistributed(layers.Embedding(output_dim=config.Params.position_emb,
    ...
    layers_to_concat = []
    num_units = config.Params.units1 * (2 if config.Params.bidirectional else 1)
    ...
    # Apply softmax
    edge_vectors = layers.Dropout(config.Params.dropout1)(edge_vectors)
    main_output = layers.wrappers.TimeDistributed(layers.Dense(n_out, activation="softmax", name='main_output'))(
        edge_vectors)

    model = models.Model(inputs=[sentence_input, entity_markers], outputs=[main_output])
    model.compile(optimizer=config.Params.optimizer, loss=masked_categorical_crossentropy, metrics=['accuracy'])

    return model


def model_ContextWeighted(embedding_matrix, max_sent_len, n_out):
    # print("Parameters:", p)
    print(config.params_dict)
    # Take sentence encoded as indices and convert it to embeddings
    sentence_input = layers.Input(shape=(max_sent_len,), dtype='int32', name='sentence_input')
...
def model_LSTMContext(embedding_matrix, max_sent_len, n_out):
    print(config.params_dict)
...
def to_indices_with_extracted_entities(graphs, word2idx):
    ...
    for index, g in enumerate(tqdm.tqdm(graphs, ascii=True)):
        ...
        for j, edge in enumerate(g["edgeSet"][:MAX_EDGES_PER_GRAPH]):
            entity_markers = [m for _, m in
                              graph_utils.get_entity_indexed_vector(g["tokens"], edge, mode=POSITION_EMBEDDING_MODE)]
